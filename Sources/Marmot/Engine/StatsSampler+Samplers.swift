...
// The individual metric samplers. Instance methods use working state on the
// class and always run on its serial sample queue.
extension StatsSampler {
    ...
    func sampleNetwork(previousHistory: NetworkStats) -> NetworkStats {
    ...
    func sampleProcesses() -> [ProcessStat] {
        let now = Date()
